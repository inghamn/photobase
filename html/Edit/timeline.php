...
<tr><td width="100">
		...
		<select name="year" onChange="document.year.submit();">
			...
			<?php
				...
				while ($row = mysql_fetch_array($result))
				{
					...
					else { echo "<option>$row[year]</option>"; }
				}
			?>
		</select>
		</form>
	...
	<td><form name="location" method="post" action="setLocation.php">
		<select name="state" onChange="document.location.submit();">
			...
			<?php
				...
				while ($row = mysql_fetch_array($result))
				{
					if (isset($_SESSION['STATE']) && $_SESSION['STATE'] == $row['state']) { echo "<option selected>$row[state]</option>"; }
	...
	<td><form name="keywords" method="post" action="setKeywords.php">
		...
		<input type="image" src="../interface/search.gif" align="middle">
		</form>
	</td>
</tr>
</table>